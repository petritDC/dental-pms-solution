...
exports.callNanokCariesInference = callNanokCariesInference;
exports.requireNanokApiKey = requireNanokApiKey;
const validateImage_1 = require("./validateImage");
async function callNanokCariesInference(params) {
    ...
    form.append("image", new Blob([bytes], { type: "image/jpeg" }), filename?.endsWith(".jpg") || filename?.endsWith(".jpeg") ? filename : "xray.jpg");
...
function requireNanokApiKey() {
    const apiKey = process.env.NANOOK_API_KEY;
    if (!apiKey) {
        throw new validateImage_1.HttpError(500, "MISSING_ENV", "NANOOK_API_KEY is required");
    }
    return apiKey;
}
